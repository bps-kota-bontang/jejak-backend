...
func RegisterSurveyRoutes(router fiber.Router, handler *handlers.SurveyHandler) {
	...
	surveys.Get("/:surveyPeriodId/regions/filter-options", handler.GetRegionFilterOptions)
	surveys.Get("/:surveyPeriodId/regions", handler.GetRegions)
	surveys.Get("/:surveyPeriodId/regions/contacts/template", handler.DownloadSurveyRegionContactsTemplate)
	...
	surveys.Post("/:surveyPeriodId/regions/contacts/import", handler.ImportSurveyRegionContacts)
